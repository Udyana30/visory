...
import { formatDateTime } from '../lib/stt.utils';
...
const HistoryCard: React.FC<HistoryCardProps> = ({ item, isCurrentlyPlaying, onPlay, onDelete }) => {
  return (
    <div
      onClick={() => onPlay(item)}
      className={`group relative bg-gray-50 border rounded-lg p-4 transition-all hover:shadow-md cursor-pointer ${
        isCurrentlyPlaying ? 'border-black bg-gray-100 ring-2 ring-black ring-opacity-20' : 'border-gray-200'
      }`}
    >
