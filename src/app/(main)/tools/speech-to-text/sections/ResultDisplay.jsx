...
import { Bot, Download, AlertCircle, CheckCircle, FileText, FileAudio } from 'lucide-react';
      ...
      {result && !isLoading && (
        <div className="space-y-5">
          <div className="flex items-center gap-2 text-green-600 bg-green-50 px-3 py-2 rounded-lg border border-green-200">
            <CheckCircle size={18} />
            <span className="text-sm font-semibold">Transcription Successful</span>
          </div>
          ...
          <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
            <p className="text-sm font-semibold text-blue-900 mb-3">Transcription Result</p>
          ...
          <div className="text-sm text-gray-600 space-y-2 pt-3 border-t border-gray-200">
            <div className="flex justify-between">
              ...
              <span className="text-gray-900 uppercase">{result.language}</span>
            </div>
            <div className="flex justify-between">
          ...
          <div className="grid grid-cols-2 gap-3">
            <a 
              ...
              download
              className="flex items-center justify-center gap-2 px-4 py-3 bg-white border-2 border-gray-900 text-gray-900 font-bold rounded-lg hover:bg-gray-50 transition-all"
            >
              <FileText size={18} />
              Download SRT
            </a>
            <a 
              href={result.input_audio_url || '#'}
              download
              className="flex items-center justify-center gap-2 px-4 py-3 bg-gray-900 text-white font-bold rounded-lg hover:bg-gray-800 transition-all shadow-md hover:shadow-lg"
            >
              <FileAudio size={18} />
              Download Audio
            </a>
      ...
      {!isLoading && !error && !result && (
        <div className="text-center py-12">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-gray-500 font-medium">No transcription yet</p>
            <p className="text-sm text-gray-400">Upload audio and click generate</p>
          </div>
        </div>
      )}
    </div>
