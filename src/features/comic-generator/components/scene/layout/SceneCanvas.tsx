...
import { Image as ImageIcon, ZoomIn, ZoomOut, Maximize, Sparkles } from 'lucide-react';
...
export const SceneCanvas: React.FC<SceneCanvasProps> = ({ scene, isGenerating, isFirst }) => {
    ...
    <div className="relative w-full h-full flex items-center justify-center bg-gray-100/50 overflow-hidden group">
      ...
      >
        {isGenerating ? (
          ...
          <div className="aspect-video w-[480px] flex flex-col items-center justify-center bg-white border border-gray-200 shadow-sm rounded-xl p-8 text-center transition-colors">
            <div className="w-14 h-14 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl flex items-center justify-center mb-5 text-blue-600 shadow-sm border border-blue-100">
              <Sparkles className="w-7 h-7" />
            </div>
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Start Your Story</h3>
            <p className="text-gray-500 mb-6 text-sm max-w-[320px]">
              Create your first scene to begin visualizing your comic. Describe the setting, characters, and mood.
            </p>
          </div>
        ) : (
