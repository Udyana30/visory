  ...
  const renderSelect = (label: string, field: keyof SceneVisualization, options: string[]) => (
    <div>
      ...
      <select
        ...
        disabled={disabled}
        className="w-full px-2.5 py-1.5 border border-gray-300 rounded-md text-xs text-gray-800 focus:ring-1 focus:ring-teal-500 bg-white disabled:bg-gray-100 disabled:text-gray-400"
      >
        {options.map(opt => (
